...
class PixelMatrix {
  constructor (width, height, colorProfile = RGBA, pixels) {
    ...
    if (pixels === undefined) {
      ...
      if (pixelsLength !== pixels.length) {
        throw new Error(
          `Expected pixels to have length ${pixelsLength} (width * height * colorProfile.channels) but got ${pixels.length} instead.`
        )
      }
    }
    this.pixels = pixels
  ...
  getIndex (point) {
    ...
    if (!this.contains(point)) {
      throw new Error(
        `Expected x and y to be less than or equal to (${this.width}, ${this.height}) but was actually (${x}, ${y})`
      )
    }
  ...
  putPixels (canvas, ImageData = window.ImageData) {
    if (canvas.width !== this.width || canvas.height !== this.height) {
      throw new Error(
        `Expected canvas shape and PixelMatrix shape to be the same, but canvas shape was [${canvas.width}, ${canvas.height}] and PixelMatrix shape was ${this.shape}.`
      )
    }
    const context = canvas.getContext('2d')
    // const imageData = context.getImageData(0, 0, canvas.width, canvas.height)
    ...
    // }
    context.putImageData(this.toImageData(ImageData), 0, 0)
  }
  getCenter () {
